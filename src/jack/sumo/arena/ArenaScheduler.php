        ...
        switch ($this->plugin->phase) {
            case Arena::PHASE_LOBBY:
                if(count($this->plugin->players) >= 2) {
                    $this->plugin->broadcastMessage("§eStarting in§4 " . Time::calculateTime($this->startTime) . " §esec.", Arena::MSG_TIP);
                    $this->startTime--;
                    if($this->startTime == 0) {
                    ...
                    $this->plugin->broadcastMessage("§cYou need more players to start the game!", Arena::MSG_TIP);
                    $this->startTime = 5;
                }
                break;
            case Arena::PHASE_GAME:
                $this->plugin->broadcastMessage("§eEnding in:§4 " . Time::calculateTime($this->gameTime) . "", Arena::MSG_TIP);

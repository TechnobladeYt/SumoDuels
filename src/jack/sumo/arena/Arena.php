...
class Arena implements Listener {
    ...
    public function joinToArena(Player $player) {
        if(!$this->data["enabled"]) {
            $player->sendMessage("§cThe game is under setup!");
            return;
        }
        ...
        if($this->inGame($player)) {
            $player->sendMessage("§cYou are already in waiting queue!");
            return;
        }
        ...
        $this->broadcastMessage("§l§bGame §r§l>> §r§ePlayer {$player->getName()} joined the match! §7[".count($this->players)."/{$this->data["slots"]}]");
    ...
    public function disconnectPlayer(Player $player, string $quitMsg = "", bool $death = \false) {
        ...
        if(!$death) {
            $this->broadcastMessage("§l§bGame §r§l>> §r§ePlayer {$player->getName()} left the match. §7[".count($this->players)."/{$this->data["slots"]}]");
        }
    ...
    public function startGame() {
        ...
        $this->broadcastMessage("§cGO", self::MSG_TITLE);
    }
    ...
    public function startRestart() {
        ...
        $player->addTitle("§bY§aO§bU §cW§5O§4N§1!");
        $this->plugin->getServer()->getPluginManager()->callEvent(new PlayerArenaWinEvent($this->plugin, $player, $this));
        $this->plugin->getServer()->broadcastMessage("§l§bSumo §r§l>> §r§ePlayer {$player->getName()} won the match at {$this->level->getFolderName()}!");
        $this->phase = self::PHASE_RESTART;
    }
    ...
    public function onLevelChange(EntityLevelChangeEvent $event) {
        ...
        if(!$player instanceof Player) return;
        if($this->inGame($player)) {
            $this->disconnectPlayer($player, "You left the game.");
        }
    }
